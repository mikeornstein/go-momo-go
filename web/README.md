...
| Start / Pause (chrome bars, Esc, P) | **Splash:** start day 1. **Play:** pause. **Pause:** resume. |
| A / Enter / R | **Splash, pause, and end screens.** Splash: start day 1. Pause: **Resume**. After a **win**: next day. After a **loss**: back to **day 1** / first neighborhood. During play: does nothing. |
| B / N | **Splash, pause, and end screens.** Splash: start day 1 (same neighborhood). Pause: **Restart at day 1**. After a **win**: next day (new neighborhood). After a **loss**: new neighborhood at **day 1**. During play: does nothing. |
...
A 1-bit splash waits for A/B (or the chrome start/pause control) before the clock starts. Art card `splash.png` is the full 400×240 Imagine next-pass title card (**GO MOMO GO** / **Poop. Then home.**, Momo lower-left, walker, taut leash, wall cat). Start hints use the post-dither bitmap font. Fallback doodle is marked `tmp splash` if the PNG 404s.
