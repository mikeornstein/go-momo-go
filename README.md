...
**Status:** Walk, leash, and the go loop are in. After a poo, press **A** next to the pile to bag it or home fails with *You left it.* Pee-mail and encounters are next.
...
- `source` - Game code and assets
  ...
  - `walk/surfaces.lua` / `hud.lua` / `mess.lua` - Grass, meters, puddles, piles, bagging
